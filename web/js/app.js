...
function sourceCard(s){return '<div class="card"><h3>'+esc(s.name)+'</h3><p class="muted">'+esc(s.url)+'</p><p>'+(s.apps||[]).length+' apps</p><button onclick="showSource(\''+encodeURIComponent(s.url)+'\')">Browse</button> <button onclick="editSource(\''+encodeURIComponent(s.url)+'\')">Edit</button> <button onclick="sourceLinks(\''+encodeURIComponent(s.url)+'\')">Add to…</button> <button onclick="removeSource(\''+encodeURIComponent(s.url)+'\')">Remove</button></div>'}
